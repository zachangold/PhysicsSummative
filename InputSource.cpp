 ...
 * and keyboard input.
 */
bool InputSource::init( HWND *hWnd ) {
    this->hWnd = hWnd;
 ...
 */
bool InputSource::initDirectInput() {
...
bool InputSource::handleMessages( UINT message, WPARAM wParam, LPARAM lParam ) {
    switch( message ) {
        case WM_KEYUP: {
            ...
            {
                quit = true;
            } else if ( wParam == 'Q' && !inputState->getKey( wParam ) )
            {
                paused = !paused;
            } else if ( wParam == 'R' && !inputState->getKey( wParam ) )
            {
                slowdown = !slowdown;
            } else if ( wParam == 'T' && !inputState->getKey( wParam ) )
            {
                speedup = !speedup;
            } else if ( wParam >= '1' && wParam <= ( '1' + NUM_SCENES - 1 ) && !inputState->getKey( wParam ) )
            {
                quit = false;
                paused = true;
                slowdown = false;
