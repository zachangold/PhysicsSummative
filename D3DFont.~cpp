 ...
 *
 */
void D3DFont::init( LPDIRECT3DDEVICE9 device, int screenWidth, int screenHeight )
{
    HRESULT hr;
 ...
 */
void D3DFont::setText( std::string newText )
{
    text = newText;
};
 ...
 *      screen.
 */
void D3DFont::render( LPDIRECT3DDEVICE9 device, unsigned int color, int x, int y, unsigned int drawFlags, int w, int h )
{
    RECT font_rect;

    // Draw from the top of the screen
    SetRect( &font_rect, x, y, x + w, y + h );
